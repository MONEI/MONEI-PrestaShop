(function () {
    ...
    function initializeMoneiAdmin($) {
        ...
        function toggleFields() {
            const isProduction = $('input[name="MONEI_PRODUCTION_MODE"]:checked').val() === '1';
        ...
        const creditSlipHandler = {
            refundReasons:
                typeof MoneiVars !== 'undefined' && MoneiVars.refundReasons
            ...
            watchForPartialRefundButton: function () {
                ...
                $(document).on(
                    'click',
                    '.cancel-product-element button.cancel, .cancel-product-element button[name="cancel"]',
                    function () {
                ...
                const observer = new MutationObserver(() => {
                    ...
                    if (
                        $('input[id^="cancel_product_quantity_"]:visible').length > 0 &&
            ...
            interceptFormSubmission: function () {
                ...
                $(document)
                    .off('submit.monei')
                    .on('submit.monei', 'form[name="cancel_product"]', function (e) {
                        const $form = $(this);
                        const refundReason =
                            $('#monei_credit_slip_reason').val() || 'requested_by_customer';

                        // Ensure hidden input exists with current value
                        let $hiddenInput = $form.find('input[name="monei_refund_reason"]');
                        if ($hiddenInput.length === 0) {
                            $hiddenInput = $(
                                `<input type="hidden" name="monei_refund_reason" value="${refundReason}" />`
                            );
        ...
        const capturePaymentHandler = {
            ...
            handleCaptureClick: function (e) {
                ...
                $btn.prop('disabled', true).html(
                    '<i class="icon-spinner icon-spin"></i> Capturing...'
                );
