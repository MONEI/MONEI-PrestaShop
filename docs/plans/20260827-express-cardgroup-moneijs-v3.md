...
Tasks 1–18 are implemented and verified on the Flashlight stack. What was actually
built, and what was learned doing it, is recorded in the commit messages on
`feat/express-cardgroup-moneijs-v3`.

Deviations from this plan worth knowing:

- **Task 4 was smaller than planned in one way and larger in another.** `front.js`
  already dispatched to the five init functions and each already guarded on its own
  container, so no dispatch rewrite was needed. But Apple Pay's wrapper carried an
  `else` branch that hides the Apple Pay option on incapable devices, and both
  wallet functions were declared inside `if` blocks — a function declaration in a
  block statement. Both were preserved deliberately.
- **Task 7's premise changed.** `getPaymentMethodsAvailable()` turned out to have no
  callers; the live path is `getPaymentMethodsAllowed()`, which reads the MONEI API
  rather than configuration. The auth filtering and the warning were built around
  the real path.
- **Task 12 does not port the WooCommerce cart backup.** PrestaShop lets a customer
  own several carts, so product page express gets one of its own and the context is
  pointed back afterwards. Nothing is ever emptied.
- **Task 13 creates a payment, not an order.** Order creation stays with
  `OrderService` and the existing confirmation and webhook path.
- **Task 17 wraps every new string in `$this->l()` but does not regenerate the 15
  locale files.** Those keys are md5 hashes generated by PrestaShop's translation
  tool; hand writing ~375 entries would be busywork with a real risk of wrong
  hashes. The translation pass is flagged for the normal workflow.
- **Environment**: verified on **PrestaShop 9.1.4** with the `hummingbird` theme,
  not 8.x. ngrok was replaced with a Cloudflare Quick Tunnel after the free tier
  started returning `ERR_NGROK_727` partway through runs.
