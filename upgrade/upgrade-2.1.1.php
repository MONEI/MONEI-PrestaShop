...
/**
 * Upgrade to 2.1.1 - take express checkout off the payment step.
 *
...
function upgrade_module_2_1_1($module)
{
    try {
        ...
        // shipped 2.1.0 default. A merchant who chose their own set keeps it.
        if (Configuration::get('MONEI_EXPRESS_LOCATIONS') === 'product,cart,checkout') {
            if (!Configuration::updateValue('MONEI_EXPRESS_LOCATIONS', 'product,cart')) {
                Monei::logError('[MONEI] Upgrade to 2.1.1 could not update MONEI_EXPRESS_LOCATIONS');
        ...
        Monei::logDebug('[MONEI] Upgrade to 2.1.1 completed successfully');

        return true;
    } catch (Exception $e) {
        Monei::logError('[MONEI] Upgrade to 2.1.1 failed: ' . $e->getMessage());
