...
class MoneiPaymentModuleFrontController extends ModuleFrontController
{
	public $ssl = true;
	...
	public function initContent()
	{
		...
		if (!$this->module->active) {
			$errorMessage = "An error ocurred while loading MONEI Payment Gateway, please contact support (MONEI Payment Gateway is not Active)" .
					$hasError = true;
		}

		if (!isset($cart) || $cart == null && !is_a($cart, "Cart")) {
			$errorMessage = "An error ocurred while loading MONEI Payment Gateway, please contact support (There is a problem with your Cart)" .
					$hasError = true;
		...
		if (Configuration::get($this->module->prefix . 'acceptedPayment_visa') == null && Configuration::get($this->module->prefix . 'acceptedPayment_mastercard') == null)
			if (Configuration::get($this->module->prefix . 'acceptedPayment_maestro') == null && Configuration::get($this->module->prefix . 'acceptedPayment_jcb') == null) {
				$errorMessage = "An error ocurred while loading the  MONEI Payment Gateway, No payment methods allowed" .
						$hasError = true;
		...
		$this->context->smarty->assign(array(
				...
				'currentLanguage' => $this->context->language->iso_code
