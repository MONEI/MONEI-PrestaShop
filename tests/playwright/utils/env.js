 ...
 *
 * The 1.7 environment's compose file sets `name: tunnel17`, so the container is
 * `tunnel17-prestashop-1`. It differs from the PrestaShop 9 stack's `tunnel1` on
 * purpose, so both lines can run at once. Overridable because a developer may run
 * the stack under another project name.
 *
 * @return {string} Container name
 */
const containerName = () => (process.env.MONEI_E2E_PS_CONTAINER || 'tunnel17-prestashop-1').trim();

/**
 * Ask the store what domain it believes it is served on.
 *
 * The stack is fronted by a Cloudflare Quick Tunnel, so the shop domain is that
 * tunnel rather than `localhost:8017`, and the published port merely 302s to it.
 * The tunnel URL changes every time the stack restarts, which makes
...
const baseUrl = () => {
    if (cachedBaseUrl === undefined) {
        ...
        cachedBaseUrl = (configured || detectShopUrl() || 'http://localhost:8017').replace(
            /\/$/,
            ''
        );
    }
