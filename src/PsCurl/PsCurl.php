...
class PsCurl
{
    ...
    {
        $user_agent = 'PHP-Curl-Class/' . self::VERSION . ' (+https://github.com/drkbcn/ps-curl)';
    ...
    public function buildPostData($data)
    {
        ...
        if (isset($this->headers['Content-Type']) &&
            ...
            $data = \PsCurl\Encoder::encodeJson($data);
            ...
            foreach ($data as $key => $value) {
                if (is_string($value) && strpos($value, '@') === 0 && is_file(\Tools::substr($value, 1))) {
                    $binary_data = true;
                    if (class_exists('CURLFile')) {
                        $data[$key] = new \CURLFile(\Tools::substr($value, 1));
                    }
    ...
    {
        $this->jsonDecoder = '\PsCurl\Decoder::decodeJson';
    ...
    {
        $this->xmlDecoder = '\PsCurl\Decoder::decodeXml';
     ...
     * @param  $raw_headers
     *
     * @return \PsCurl\CaseInsensitiveArray
     */
    ...
    public function download($url, $mixed_filename)
    {
        ...
        if (is_callable($mixed_filename)) {
            ...
            $this->downloadCompleteCallback = function ($fh) use ($download_filename, $filename) {
    ...
    public function put($url, $data = [])
    {
        ...
        if (empty($this->options[CURLOPT_INFILE]) && empty($this->options[CURLOPT_INFILESIZE])) {
            if (is_string($put_data)) {
                $this->setHeader('Content-Length', \Tools::strlen($put_data));
            }
        }
        if (!empty($put_data)) {
    ...
    {
        $callback = function ($downloaded) use ($bytes) {
    ...
    public function setDefaultDecoder($mixed = 'json')
    {
        if ($mixed === false) {
            ...
            if ($mixed === 'json') {
                $this->defaultDecoder = '\PsCurl\Decoder::decodeJson';
            } elseif ($mixed === 'xml') {
                $this->defaultDecoder = '\PsCurl\Decoder::decodeXml';
            }
        }
    }
    ...
    public function diagnose($return = false)
    {
        ...
        if ($this->attempts === 0) {
            ...
            $response_calculated_length = is_string($this->rawResponse) ?
                \Tools::strlen($this->rawResponse) : '(' . var_export($this->rawResponse, true) . ')';
    ...
    public function setStop($callback)
    {
        ...
        $this->progress(function () use ($header_callback_data) {
